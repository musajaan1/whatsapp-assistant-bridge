 ...
 * @returns {Promise<T>}
 */
async function callWithRetry(fn, maxRetries = 3, baseDelayMs = 3000) {
  let lastError;
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      ...
      if (isRateLimit && attempt < maxRetries) {
        ...
        console.warn(`[GeminiService] Rate limit hit. Waiting ${delay / 1000}s before retry (attempt ${attempt + 1}/${maxRetries})...`);
 ...
 * @returns {Promise<{intent: "GENERAL_CHAT" | "FILE_REQUEST" | "SUMMARIZE_REQUEST", confidence: number, reasoning: string}>}
 */
export async function classifyIntent(messageText) {
  ...
  const model = ai.getGenerativeModel({
      ...
      responseSchema: {
        ...
        properties: {
          intent: {
            ...
            enum: ["GENERAL_CHAT", "FILE_REQUEST", "SUMMARIZE_REQUEST"],
    ...
    systemInstruction:
      ...
      "- 'FILE_REQUEST': The user is asking to find, search, send, open, or fetch a file, document, image, PDF, report, or spreadsheet from their computer or drives.\n" +
